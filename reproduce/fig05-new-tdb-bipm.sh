#!/usr/bin/env bash
# Generate the timing-component data used for Fig. 5: numerical LAL TT-TDB
# correction versus PINT TT(BIPM), at the paper's three sky positions.
set -euo pipefail
...
"$PYTHON" "$ROOT/scripts/checkBarycentering.py" \
  --output-dir "$ROOT/results/fig05-new-tdb-bipm" \
